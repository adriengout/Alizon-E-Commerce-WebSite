...
try {
    ...
    $stmt = $dbh->prepare('SELECT login,mail FROM enfant._compte');
    $stmt->execute();
...
function validerSiretLuhn($siret) {
    ...
    for ($i = 0; $i < 14; $i++) {
        ...
        // Pour les positions paires (0, 2, 4...), on multiplie par 2
        if ($i % 2 === 0) {
            $chiffre *= 2;
...
function validerLogin($login, $loginsExistant) {
    $login = trim($login);
...
function validerEmail($email, $mailsExistant) {
    $email = trim($email);
